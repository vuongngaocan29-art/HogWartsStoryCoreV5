...
Cầu nối giữa hai plugin là mục `cast-hooks` trong `config.yml`: mỗi lần niệm phép thành công, plugin chạy một lệnh console (mặc định là `bq action ...`) để BetonQuest đếm số lần luyện tập. Plugin cũng bắn `SpellCastEvent` cho ai muốn viết addon riêng.

 ...
 *  %hogwarts_quest_name%     Nhiệm vụ đang theo dõi
 *  %hogwarts_quest_progress% Tiến độ dạng 2/3
 *  %hogwarts_quest_bar%      Thanh tiến độ ■■■□□
 *  %hogwarts_quest_line%     Một dòng hoàn chỉnh cho TAB
 */
public class HogwartsPlaceholders extends PlaceholderExpansion {
    ...
    private int trackedDone(StudentProfile p) {
        if (!p.hasTrackedQuest()) return 0;
        return Math.max(0, Math.min(p.getTrackedQuestGoal(), p.getTrackedQuestProgress()));
    }

    private String progressBar(int done, int goal) {
        if (goal <= 0) return "&8-----";
        int slots = 10;
        int filled = (int) Math.round((done * slots) / (double) goal);
    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        ...
        if (params.startsWith("has_spell_")) {
            String id = params.substring("has_spell_".length()).replace('_', '-');
            return p.hasLearned(id) ? "true" : "false";
        }
        if (params.startsWith("practice_spell_")) {
            String id = params.substring("practice_spell_".length()).replace('_', '-');
            return p.isPracticing(id) ? "true" : "false";
        }
        ...
        return switch (params) {
            ...
            case "quest_status" -> p.hasTrackedQuest() ? "active" : "none";
            case "quest_name" -> p.hasTrackedQuest() ? p.getTrackedQuestName() : "Không có nhiệm vụ";
            case "quest_done" -> String.valueOf(p.hasTrackedQuest() ? trackedDone(p) : 0);
            case "quest_goal" -> String.valueOf(p.hasTrackedQuest() ? p.getTrackedQuestGoal() : 0);
            case "quest_progress" -> {
                if (!p.hasTrackedQuest()) yield "-";
                int done = trackedDone(p);
                yield done + "/" + p.getTrackedQuestGoal();
            }
            case "quest_bar" -> {
                if (!p.hasTrackedQuest()) yield "&8■■■■■■■■■■";
                int done = trackedDone(p);
                yield progressBar(done, p.getTrackedQuestGoal());
            }
            case "quest_line" -> {
                if (!p.hasTrackedQuest()) yield "&8Không có nhiệm vụ";
                int done = trackedDone(p);
                yield "&e" + p.getTrackedQuestName() + " &f" + done + "/" + p.getTrackedQuestGoal();
            }
            default -> null;
        };
    }

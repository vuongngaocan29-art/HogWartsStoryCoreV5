...
public class SpellManager {
    ...
    public Result cast(Player player, String spellId) {
        ...
        if (!p.hasLearned(spell.id()) && !p.isPracticing(spell.id())
                && !player.hasPermission("hogwarts.spell.bypass")) {
    ...
    private void execute(Player player, StudentProfile p, Spell spell) {
        ...
        switch (spell.id()) {
            ...
            case "lumos-maxima" -> {
                ...
                nearbyTargets(player, 8).forEach(e -> {
                    if (e instanceof LivingEntity le && !le.equals(player)) {
                        ...
                        if (le instanceof Monster) dealSpellDamage(player, le, spell, power);
                    }
                });
            }
            ...
            case "incendio" -> {
                Entity hit = rayTarget(player, 18);
                Location target = hit instanceof LivingEntity
                        ? hit.getLocation().add(0, 1, 0)
                        : eye.clone().add(eye.getDirection().multiply(6));
                w.spawnParticle(Particle.FLAME, target, 45, 0.45, 0.45, 0.45, 0.025);
                if (hit instanceof LivingEntity le && !le.equals(player)) {
                    dealSpellDamage(player, le, spell, power);
                    le.setFireTicks((int) (60 * power));
                } else {
                    nearbyAt(target, 2.5).forEach(e -> {
                        if (e instanceof LivingEntity le && !le.equals(player)) {
                            dealSpellDamage(player, le, spell, power * 0.75);
                            le.setFireTicks((int) (60 * power));
                        }
                    });
                }
            ...
            case "confringo" -> {
                Entity hit = rayTarget(player, 28);
                Location target = hit instanceof LivingEntity
                        ? hit.getLocation().add(0, 1, 0)
                        : eye.clone().add(eye.getDirection().multiply(9));
                w.spawnParticle(Particle.EXPLOSION, target, 4, 0.5, 0.5, 0.5, 0.05);
                w.playSound(target, Sound.ENTITY_GENERIC_EXPLODE, 1.3f, 0.8f);
                nearbyAt(target, 3.5).forEach(e -> {
                    if (e instanceof LivingEntity le && !le.equals(player)) {
                        double scale = (hit != null && e.getUniqueId().equals(hit.getUniqueId())) ? 1.0 : 0.65;
                        dealSpellDamage(player, le, spell, power * scale);
                    }
                });
            }
            ...
            case "immobulus" -> nearbyTargets(player, 10).forEach(e -> {
                if (e instanceof LivingEntity le && !le.equals(player)) {
                    ...
                    Bukkit.getScheduler().runTaskLater(plugin, () -> {
                        if (le.isValid() && !le.isDead()) le.setAI(true);
                    }, 120L);
                }
            });
            case "reducto" -> {
                Entity hit = rayTarget(player, 24);
                Block b = rayBlock(player, 18);
                if (b != null && plugin.getConfig().getBoolean("magic.spell-block-damage", false)) {
                    b.breakNaturally();
                }
                Location target = hit instanceof LivingEntity
                        ? hit.getLocation().add(0, 1, 0)
                        : (b != null ? b.getLocation().add(0.5, 0.5, 0.5)
                                     : eye.clone().add(eye.getDirection().multiply(8)));
                w.spawnParticle(Particle.EXPLOSION, target, 3, 0.35, 0.35, 0.35, 0.03);
                nearbyAt(target, 2.75).forEach(e -> {
                    if (e instanceof LivingEntity le && !le.equals(player)) {
                        double scale = (hit != null && e.getUniqueId().equals(hit.getUniqueId())) ? 1.0 : 0.7;
                        dealSpellDamage(player, le, spell, power * scale);
                    }
                });
            }
            ...
            case "expecto-patronum" -> {
                ...
                nearbyTargets(player, 15).forEach(e -> {
                    if (e instanceof Monster m) {
                        ...
                        dealSpellDamage(player, m, spell, power);
                    }
                });
    ...
    /**
     * Gây sát thương phép theo cấu hình spells.yml và lõi đũa.
     * 2.0 damage = 1 tim Minecraft.
     * PvP có hệ số riêng để quái có thể đủ khó mà người chơi không bị hạ quá nhanh.
     */
    private void dealSpellDamage(Player caster, LivingEntity target, Spell spell, double power) {
        if (target == null || target.equals(caster) || target.isDead() || !target.isValid()) return;
        double base = spell.damage();
        if (base <= 0) return;

        double multiplier = target instanceof Player
                ? plugin.getConfig().getDouble("magic.pvp-damage-multiplier", 0.65)
                : plugin.getConfig().getDouble("magic.mob-damage-multiplier", 1.0);

        double amount = Math.max(0.0, base * Math.max(0.1, power) * Math.max(0.0, multiplier));
        if (amount <= 0) return;

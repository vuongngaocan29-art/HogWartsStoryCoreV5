...
public class HogwartsCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        if (args.length == 0) {
            ...
            else sender.sendMessage("Dùng: /hogwarts points|sorting|year|teach|lessonreward|cup|reload");
            return true;
        }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "queststart", "nhiemvustart" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 7) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts queststart <player> <id> <goal> <point|objective|binary> <key> <tên_dùng_gạch_dưới>"));
                    return true;
                }
                ...
                try { goal = Integer.parseInt(args[3]); }
                    ...
                    sender.sendMessage(Msg.msg("&cGoal nhiệm vụ không hợp lệ."));
                    return true;
                }

                StudentProfile p = plugin.getProfiles().get(t);
                p.setTrackedQuestId(args[2]);
                p.setTrackedQuestGoal(Math.max(1, goal));
                p.setTrackedQuestSource(args[4].toLowerCase(Locale.ROOT));
                p.setTrackedQuestKey(args[5].equals("-") ? "" : args[5]);
                p.setTrackedQuestName(String.join(" ", Arrays.copyOfRange(args, 6, args.length)).replace('_', ' '));
                plugin.getProfiles().save(p);
            }

            case "questcomplete", "nhiemvucomplete" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 3) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts questcomplete <player> <id>"));
                    return true;
                }
                ...
                StudentProfile p = plugin.getProfiles().get(t);
                if (p != null && p.hasTrackedQuest()
                        && args[2].equalsIgnoreCase(p.getTrackedQuestId())) {
                    p.clearTrackedQuest();
                    plugin.getProfiles().save(p);
                }
            }

            case "questclear", "nhiemvuclear" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
            ...
            case "lessonreward", "thuongbaihoc" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 4) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts lessonreward <người chơi> <điểm-nhà> <tiền> [bài-học]"));
                    return true;
                }
                ...
                double money;
                try {
                    housePoints = Integer.parseInt(args[2]);
                    money = Double.parseDouble(args[3]);
                } catch (NumberFormatException ex) {
                    sender.sendMessage(Msg.msg("&cĐiểm Nhà hoặc số tiền không hợp lệ."));
                    return true;
                }

                if (housePoints < 0 || money < 0) {
                    sender.sendMessage(Msg.msg("&cPhần thưởng không được là số âm."));
                    return true;
                }

                StudentProfile profile = plugin.getProfiles().get(t);
                House house = profile.getHouse();

                if (house != null && housePoints > 0) {
                    String lesson = args.length > 4
                            ? String.join(" ", Arrays.copyOfRange(args, 4, args.length))
                            : "hoàn thành bài học";
                    plugin.getPoints().award(house, housePoints,
                            t.getName() + " " + lesson);
                    t.sendMessage(Msg.msg("&e+" + housePoints + " Điểm Nhà &7cho " + house.getDisplayName()));
                }

                if (money > 0) {
                    // Dùng Vault economy thông qua command eco để tương thích Essentials Economy.
                    // Console dispatch tránh thêm hard dependency Vault vào core.
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
                            "eco give " + t.getName() + " " + money);
                    t.sendMessage(Msg.msg("&6+" + ((money == Math.rint(money)) ? String.valueOf((int) money) : String.valueOf(money))
                            + " Galleon &7phần thưởng bài học."));
                }

                sender.sendMessage(Msg.msg("&aĐã trao phần thưởng bài học cho " + t.getName() + "."));
            }
            ...
            case "practice", "luyenphep" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 4) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts practice <người chơi> <phép> <on|off>"));
                    return true;
                }
                Player t = Bukkit.getPlayerExact(args[1]);
                var spell = plugin.getSpells().get(args[2]);
                if (t == null || spell == null) {
                    sender.sendMessage(Msg.msg("&cKhông tìm thấy người chơi hoặc phép."));
                    return true;
                }
                StudentProfile profile = plugin.getProfiles().get(t);
                if (args[3].equalsIgnoreCase("on")) {
                    if (profile.getYear() < spell.year()) {
                        sender.sendMessage(Msg.msg("&cNgười chơi chưa đủ năm học cho phép này."));
                        return true;
                    }
                    profile.startPractice(spell.id());
                    if (profile.getActiveSpell() == null) profile.setActiveSpell(spell.id());
                    plugin.getProfiles().save(profile);
                    t.sendMessage(Msg.msg("&e✦ Chế độ luyện tập: &f" + spell.incantation()
                            + "&e. Hoàn thành bài học để mở khoá vĩnh viễn."));
                    sender.sendMessage(Msg.msg("&aĐã cho " + t.getName() + " luyện " + spell.incantation() + "."));
                } else if (args[3].equalsIgnoreCase("off")) {
                    profile.stopPractice(spell.id());
                    if (spell.id().equals(profile.getActiveSpell()) && !profile.hasLearned(spell.id())) {
                        profile.setActiveSpell(null);
                    }
                    plugin.getProfiles().save(profile);
                    sender.sendMessage(Msg.msg("&aĐã kết thúc luyện tập " + spell.incantation() + " của " + t.getName() + "."));
                } else {
                    sender.sendMessage(Msg.msg("&cChỉ dùng on hoặc off."));
                }
            }
            ...
            case "xp", "magicxp" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 3) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts xp <người chơi> <số XP>"));
                    return true;
                }
                Player t = Bukkit.getPlayerExact(args[1]);
                if (t == null) { sender.sendMessage(Msg.msg("&cKhông tìm thấy người chơi.")); return true; }
                int amount;
                try { amount = Integer.parseInt(args[2]); }
                catch (NumberFormatException ex) { sender.sendMessage(Msg.msg("&cXP không hợp lệ.")); return true; }
                if (amount < 0) { sender.sendMessage(Msg.msg("&cXP phải từ 0 trở lên.")); return true; }
                StudentProfile profile = plugin.getProfiles().get(t);
                int before = profile.getMagicLevel();
                profile.addXp(amount);
                plugin.getProfiles().save(profile);
                t.sendMessage(Msg.msg("&d+" + amount + " Magic XP &8· &7Cấp phép: &f" + profile.getMagicLevel()));
                if (profile.getMagicLevel() > before) {
                    t.playSound(t.getLocation(), org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 1f, 1.4f);
                }
                sender.sendMessage(Msg.msg("&aĐã cộng " + amount + " Magic XP cho " + t.getName() + "."));
            }

            case "unteach", "quenphep" -> {
                if (!sender.hasPermission("hogwarts.admin")) { noPerm(sender); return true; }
                if (args.length < 3) {
                    sender.sendMessage(Msg.msg("&7Dùng: /hogwarts unteach <người chơi> <phép>"));
                    return true;
                }
                Player t = Bukkit.getPlayerExact(args[1]);
                var spell = plugin.getSpells().get(args[2]);
                ...
                StudentProfile profile = plugin.getProfiles().get(t);
                profile.unlearn(spell.id());
                profile.stopPractice(spell.id());
                if (spell.id().equals(profile.getActiveSpell())) profile.setActiveSpell(null);
                plugin.getProfiles().save(profile);
                sender.sendMessage(Msg.msg("&aĐã xoá phép " + spell.incantation() + " khỏi " + t.getName() + "."));
            }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        if (args.length == 1) {
            ...
            if (sender.hasPermission("hogwarts.admin")) {
                base.addAll(List.of("award", "year", "teach", "practice", "xp", "unteach", "cup", "sethouse", "resort", "debug", "reload"));
            }
        ...
        if (args.length == 3 && (args[0].equalsIgnoreCase("practice") || args[0].equalsIgnoreCase("unteach"))) {
            return plugin.getSpells().ids().stream()
                    .filter(sp -> sp.startsWith(args[2].toLowerCase())).toList();
        }
        if (args.length == 4 && args[0].equalsIgnoreCase("practice")) {
            return List.of("on", "off").stream()
                    .filter(v -> v.startsWith(args[3].toLowerCase())).toList();
        }

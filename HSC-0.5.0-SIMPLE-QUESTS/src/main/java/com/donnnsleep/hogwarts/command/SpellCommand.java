...
public class SpellCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        String incantationText = castNow && args.length > 1
                ? String.join(" ", java.util.Arrays.copyOfRange(args, 1, args.length))
                : String.join(" ", args);
        if (spell == null) spell = plugin.getSpells().byIncantation(incantationText);
        if (spell == null) {
        ...
        if (!p.hasLearned(spell.id()) && !p.isPracticing(spell.id())
            ...
            player.sendMessage(Msg.msg("&cCon chưa học &f" + spell.incantation()
                    + "&c. Hãy tìm giáo sư phụ trách để bắt đầu bài học."));
            return true;
        }
        ...
        if (castNow) {
            ...
            String training = p.isPracticing(spell.id()) && !p.hasLearned(spell.id())
                    ? " &e[ĐANG LUYỆN]" : "";
            String damageText = spell.damage() > 0
                    ? " &8· &c" + String.format(java.util.Locale.US, "%.1f", spell.damage()) + " damage"
                    : "";
            player.sendMessage(Msg.msg("&7Phép đang chọn: " + spell.displayColor() + spell.incantation()
                    + " &8· &7" + (int) spell.manaCost() + " ma lực" + damageText + training));
        }
        return true;
    }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        if (args.length == 1) {
            java.util.LinkedHashSet<String> available = new java.util.LinkedHashSet<>(p.getLearnedSpells());
            available.addAll(p.getPracticeSpells());

...
public class ProfileStore {
    ...
    public StudentProfile load(UUID uuid, String name) {
        ...
        if (f.exists()) {
            ...
            p.getPracticeSpells().addAll(cfg.getStringList("practice-spells"));
            p.getStoryFlags().addAll(cfg.getStringList("story-flags"));
            p.setTrackedQuestId(cfg.getString("quest.id"));
            p.setTrackedQuestName(cfg.getString("quest.name"));
            p.setTrackedQuestSource(cfg.getString("quest.source"));
            p.setTrackedQuestKey(cfg.getString("quest.key"));
            p.setTrackedQuestGoal(cfg.getInt("quest.goal", 0));
            p.setTrackedQuestProgress(cfg.getInt("quest.progress", 0));
        }
        cache.put(uuid, p);
        return p;
    }
    ...
    public void save(StudentProfile p) {
        ...
        cfg.set("practice-spells", new ArrayList<>(p.getPracticeSpells()));
        cfg.set("story-flags", new ArrayList<>(p.getStoryFlags()));
        cfg.set("quest.id", p.getTrackedQuestId());
        cfg.set("quest.name", p.getTrackedQuestName());
        cfg.set("quest.source", p.getTrackedQuestSource());
        cfg.set("quest.key", p.getTrackedQuestKey());
        cfg.set("quest.goal", p.getTrackedQuestGoal());
        cfg.set("quest.progress", p.getTrackedQuestProgress());
        try {
            cfg.save(f);
        } catch (IOException e) {
